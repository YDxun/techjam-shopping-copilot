...
class EnvFallbackTest(unittest.TestCase):
    def test_agent_records_privacy_safe_session_observability(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            agent.respond("observed", "I need cotton", turn=1, top_k=10)

            log = agent.session_logs["observed"]
            self.assertEqual(len(log["turns"]), 1)
            self.assertGreaterEqual(log["latency_ms"], 0.0)
            self.assertIn("retrieve_ms", log["phase_timings"])
            self.assertNotIn("I need cotton", json.dumps(log))
