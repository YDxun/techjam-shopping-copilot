...
class StrategyConfig(BaseModel):
    ...
    retrieval_pool_size: int = Field(default=50, ge=10, le=500, description="候选池大小：普通50 / RECOVER=100")
...
class SessionState(BaseModel):
    ...
    constraints: dict[str, Any] = Field(default_factory=dict, description="结构化约束，上层已处理 override 清空")
...
class PipelineOutput(BaseModel):
    ...
    raw_fused_candidates: list[tuple[str, float]] = Field(default_factory=list, description="融合后未重排 (parent_asin, fused_score)")
    reranked_top10: list[str] = Field(default_factory=list, description="重排完成 Top-10，唯一不重复")
