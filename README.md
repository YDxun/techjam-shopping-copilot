...
**不修改官方评估器**，完全兼容官方 Python Agent 接口与机器可读 API 契约；兼容默认保持
`emit_gate=false`、Hybrid 关闭与完整 `top_k` 输出，并可在零外部依赖下运行。下表是启用输出门控的
opt-in 留出集 benchmark overlay，不是当前兼容默认配置的评分。

| 指标 | 弱 BM25 基线 | 本项目（dev 公开集 200，opt-in 门控 overlay，离线） |
|---|---|---|
...
## 输出门控 benchmark overlay（2026-08-30，public 200 A/B + 40 条留出验证；opt-in、非默认配置）
...
- 该 opt-in benchmark overlay 为：`fingerprint.enable=true`、`emit_gate=true`、`emit_k0=1`、`emit_k1=1`、`emit_k2=1`、
...
- **40 条留出验证（该 opt-in overlay）**：调参（160）与留出（40）TS 均≈0.955、HR=1.0，非纯 public 过拟合。
...
|---|---|---|---|
| 规则-only（门控 benchmark overlay） | 0.955 | **0.383**（崩塌） | 0.383 |
...
- 在该 opt-in 门控 benchmark overlay 中，模板化消息 → 规则路径≈0.955；若改写 → 规则会退化，LLM 需联网（已在提交说明中披露）。这不是当前兼容默认配置的评分。
...
| 配置 | HR@10 | MRR | MTTC | TS |
|---|---|---|---|---|
| 无 combo（上一版 benchmark 基线） | 1.0 | 0.6438 | 1.72 | 0.8787 |
