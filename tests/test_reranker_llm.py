...
class RerankerLLMTest(unittest.TestCase):
    ...
    def test_skipped_paths_do_not_call_chat_and_zero_usage(self) -> None:
        retriever, candidates, state, route, rule_order = make_rule_case()
        cases = (
            (env_with_rerank(False), FakeAvailableClient(LLMResult(True, "deepseek", "model")), candidates),
            (env_with_rerank(True), FakeUnavailableClient(), candidates),
            (env_with_rerank(True), FakeAvailableClient(LLMResult(True, "deepseek", "model")), candidates[:1]),
        )
        for env, client, case_candidates in cases:
            with self.subTest(client=type(client).__name__, candidates=len(case_candidates)):
                ...
                actual = reranker.rerank(retriever, case_candidates, state, route, top_k=10, mode="probe")
    ...
    def test_payload_is_compact_and_excludes_non_ranking_context(self) -> None:
        ...
        client = FakeAvailableClient(LLMResult(True, "deepseek", "deepseek-chat", content='["B", "A"]'))
    ...
    def test_valid_response_forms_filter_unknown_duplicates_and_append_rule_order(self) -> None:
        ...
        retriever.products.update({
            "C": {"parent_asin": "C", "title": "Gamma", "categories": [], "features": [], "rating_number": 0, "average_rating": 0},
            "D": {"parent_asin": "D", "title": "Delta", "categories": [], "features": [], "rating_number": 0, "average_rating": 0},
        })
        forms = (
        ...
        for provider in ("deepseek", "openai"):
            for content in forms:
                with self.subTest(provider=provider, content=content):
                    client = FakeAvailableClient(LLMResult(
                        True, provider, f"{provider}-model", content=content,
                    ...
                    self.assertEqual(reranker.last_usage, {"prompt_tokens": 11, "completion_tokens": 7})
                    self.assertEqual([item["parent_asin"] for item in user_payload(client)["candidates"]], ["A", "B"])
