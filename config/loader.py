...
from config.models import AppConfig, CircuitBreakerConfig, LLMConfig, RetryConfig
...
def _reject_secret_fields(data: Mapping[str, Any], source: str) -> None:
    llm = data.get("llm")
    if isinstance(llm, Mapping) and "api_key" in llm:
        raise ConfigError(f"{source} may not set llm.api_key; use DEEPSEEK_API_KEY")
...
def _environment_overrides(env: Mapping[str, str]) -> dict[str, Any]:
    ...
    flat_fields: dict[str, tuple[str, Callable[[str, str], Any]]] = {
        ...
        "LLM_BACKEND": ("llm_backend", _parse_text),
        ...
        "LLM_RERANK": ("llm_rerank", _parse_bool),
        ...
        "RERANK_CANDIDATES": ("rerank_candidates", _parse_int),
    ...
    nested_fields: dict[str, tuple[tuple[str, ...], Callable[[str, str], Any]]] = {
        "LLM_PROVIDER": (("llm", "provider"), _parse_text),
        "LLM_MODEL": (("llm", "model"), _parse_text),
        "LLM_BASE_URL": (("llm", "base_url"), _parse_text),
    ...
    for name, (path, parser) in nested_fields.items():
        value = _environment_value(env, name)
        if value is not None:
            _set_nested(result, path, parser(value, name))
    return result
...
def _build_and_validate(data: Mapping[str, Any]) -> AppConfig:
    ...
    llm = LLMConfig(
        ...
        model=_string_value(llm_data.get("model"), "llm.model"),
        base_url=_string_value(llm_data.get("base_url"), "llm.base_url"),
        ...
        api_key=_string_value(llm_data.get("api_key", ""), "llm.api_key"),
    )
    config = AppConfig(
        env_mode=_string_value(data.get("env_mode"), "env_mode"),
        llm_backend=_string_value(data.get("llm_backend"), "llm_backend"),
        ...
        llm_rerank=_bool_value(data.get("llm_rerank"), "llm_rerank"),
        ...
        rerank_candidates=_int_value(data.get("rerank_candidates"), "rerank_candidates"),
...
def _validate(config: AppConfig) -> None:
    ...
    _in(config.llm_backend, "llm_backend", {"none", "local", "openai"})
    ...
    _in(config.llm.provider, "llm.provider", {"none", "deepseek"})
    _positive(config.top_k, "top_k")
    _positive(config.rerank_candidates, "rerank_candidates")
