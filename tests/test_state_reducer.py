...
class StateReducerTest(unittest.TestCase):
    ...
    def test_replace_intent_increments_version_and_keeps_old_as_soft(self) -> None:
        ...
        self.assertEqual(result.state.removed_constraints, ())

    def test_replace_intent_with_erase_archives_old_constraints(self) -> None:
        erasing_reducer = StateReducer(override_erase=True)
        initial = erasing_reducer.new_state("s1", {})
        with_old_preference = erasing_reducer.reduce(
            initial,
            recognition(
                DialogueAct.ADD_CONSTRAINT,
                operation(OperationKind.ADD, "style", "casual"),
        ...
        result = erasing_reducer.reduce(with_old_preference, replacement, turn=3)
