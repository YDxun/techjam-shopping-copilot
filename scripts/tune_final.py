...
retriever = HybridRetriever(
    catalog_path=ROOT / "data" / "catalog.jsonl", env=base_env, backend=base_env.retrieval_backend
)
base_agent = Agent(
    env=base_env,
    retriever=retriever,
...
for label, ov in CANDIDATES:
    ...
    print(
        f"{label:18s} tune160 ts={t['recommended_technical_score']:.4f} mrr={t['mrr']:.4f} "
        f"| valid40 ts={v['recommended_technical_score']:.4f} mrr={v['mrr']:.4f}",
        flush=True,
    )
